...
class BlocklistUpdater:
    def __init__(self):
        ...
        self.default_config = {
            ...
            "additional_domains": [
                ...
                "nairobiraha.com", "nairobihot.com", "nairobi-escorts.com",
                "kenyacupid.com", "kenyatopescorts.com", "hotkenya.com",
                "kenyagirls.net", "kenyaxrated.com", "mombasaplaymates.com",
                "kenyaicuts.net", "escortskenya.org", "mombasahot.com",
                
                # PUCs (Public Upskirt Content) sites
                "upskirtcollection.com", "voyeurforum.com", "upskirt-times.com",
                "candid-forum.net", "candidboard.com", "upskirt-voyeur.net",
                "voyeurpapa.com", "upskirtsource.com", "candidspot.net",
                "voyeurjapantv.com", "realwifestories.com", "voyeurhit.com",
                "upskirtjerk.org", "candidshores.com", "voyeurweb.com",
                "voyeurbeach.net", "upskirtcity.net", "candidcafe.net"
            ]
        }
    ...
    def update_from_sources(self) -> Optional[Set[str]]:
        ...
        try:
            ...
            if all_domains:
                ...
                self.logger.info(f"Successfully updated blocklist with {len(all_domains)} domains")
                return all_domains
            else:
    ...
    def _save_domains(self, domains: Set[str]):
        ...
        try:
            ...
            self.logger.info(f"Successfully saved {len(domains)} domains to {self.blocklist_file}")
            return True
        except Exception as e:
    ...
    def is_update_needed(self) -> bool:
        ...
        try:
            ...
            if now - last_update > update_interval:
                self.logger.info(f"Update interval ({update_interval}) exceeded, update needed")
                return True
    ...
    def get_stats(self) -> Dict:
        ...
        try:
            ...
            stats["enabled_sources"] = sum(1 for s in self.config['sources'] if s.get('enabled', True))
    ...
    def categorize_domains(self, domains: Set[str]) -> Dict[str, Set[str]]:
        ...
        category_keywords = {
            "adult": [
                "porn", "xxx", "adult", "sex", "nude", "naked", "hentai",
                "xvideos", "pornhub", "xnxx", "xhamster", "redtube", "youporn"
            ],
            "gambling": [
                "gambling", "casino", "bet", "poker", "roulette", "slot",
                "blackjack", "bingo", "lottery", "wager"
            ],
            "social": [
                "reddit", "twitter", "instagram", "tiktok", "facebook",
                "snapchat", "tumblr", "pinterest", "telegram"
            ],
            "streaming": [
                "stream", "video", "tube", "cam", "live", "webcam", "onlyfans",
                "chaturbate", "cam4"
            ],
            "dating": [
                "date", "dating", "match", "meet", "hookup", "friend", "flirt",
    ...
    def export_blocklist(self, format_type: str, output_dir: Optional[str] = None) -> str:
        """
        ...
        try:
            ...
            if format_type == 'hosts':
                ...
                output_file = os.path.join(output_dir, f"blocklist_hosts_{timestamp}.txt")
                ...
                output_file = os.path.join(output_dir, f"blocklist_domains_{timestamp}.txt")
                with open(output_file, 'w', encoding='utf-8') as f:
                    for domain in sorted(domains):
                        f.write(f"{domain}\n")
                ...
                output_file = os.path.join(output_dir, f"blocklist_adblock_{timestamp}.txt")
                with open(output_file, 'w', encoding='utf-8') as f:
    ...
    def _extract_domains_from_compressed(self, content: bytes, format_type: str) -> Set[str]:
        """
                        ...
                        for file in files:
                            ...
                            if file in ('domain', 'urls'):
                                ...
                                try:
                                    ...
                                    self.logger.warning(f"Error processing file in archive: {file_path}, {e}")
                            ...
                            try:
                                with zip_file.open(file_name) as f:
                                    ...
                                    if file_name.endswith('.txt') or file_name.endswith('.list'):
                                        domains.update(self._extract_domains_from_list(file_content))
                                    elif file_name.endswith('.hosts'):
                                        domains.update(self._extract_domains_from_hosts(file_content))
                                    else:
                                        # Try to parse as domain list by default
                                        domains.update(self._extract_domains_from_list(file_content))
                            except Exception as e:
                                self.logger.warning(f"Error processing file in zip: {file_name}, {e}")
            
            self.logger.info(f"Extracted {len(domains)} domains from {format_type} archive")
            return domains
