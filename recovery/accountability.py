...
class AccountabilityBot:
    def __init__(self):
        ...
        # Data directory
        self.data_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'data', 'accountability')
        ...
        self.default_config = {
            ...
            "motivation_messages": [
                "Remember why you started this journey. You're stronger than your urges!",
    ...
    def set_email(self, email: str, smtp_server: str = "smtp.gmail.com", 
                  smtp_port: int = 587, username: str = "", password: str = "") -> bool:
        """
        ...
        try:
            ...
            self.config["email_settings"].update({
                ...
                "to_emails": [email] if email not in self.config["email_settings"]["to_emails"] else self.config["email_settings"]["to_emails"]
            })
    ...
    def send_email(self, subject: str, message: str, is_emergency: bool = False) -> bool:
        """
        ...
        try:
            ...
            msg['Subject'] = f"{'🚨 URGENT - ' if is_emergency else ''}Adult Content Blocker: {subject}"
            
            # Build message body
            body = f"""
Adult Content Blocker - Accountability Report

{message}

---
Sent automatically by your Adult Content Blocker accountability system.
"""
            ...
            self.logger.log_recovery_action("email_sent", {
                "subject": subject,
    ...
    def send_telegram_message(self, message: str, is_emergency: bool = False) -> bool:
        ...
        try:
            ...
            # Build message
            full_message = f"{'🚨 URGENT 🚨\\n\\n' if is_emergency else ''}🛡️ Adult Content Blocker\\n\\n{message}"
            
            if is_emergency:
                full_message += "\\n\\n🆘 Emergency Resources:\\n"
                for resource in self.config["emergency_resources"][:3]:  # First 3 resources
            ...
            for chat_id in telegram_settings["chat_ids"]:
                ...
                if response.status_code == 200:
                    ...
                    self.logger.error(f"Failed to send Telegram message to {chat_id}: {response.text}")
            
            if success_count > 0:
                self.logger.log_recovery_action("telegram_sent", {
    ...
    def send_motivation_message(self) -> bool:
        ...
        try:
            ...
            email_sent = self.send_email(subject, full_message) if self.config["email_enabled"] else True
            telegram_sent = self.send_telegram_message(full_message) if self.config["telegram_enabled"] else True
    ...
    def send_streak_milestone_alert(self, streak_days: int, milestone_message: str) -> bool:
        """
...
"""
            ...
            # Send via email and/or Telegram
            email_sent = self.send_email(subject, message) if self.config["email_enabled"] else True
            telegram_sent = self.send_telegram_message(message) if self.config["telegram_enabled"] else True
...
"""
            ...
            email_sent = self.send_email(subject, message, is_emergency=True) if self.config["email_enabled"] else True
            telegram_sent = self.send_telegram_message(message, is_emergency=True) if self.config["telegram_enabled"] else True
...
"""
    ...
    def get_config_status(self) -> Dict:
        ...
        return {
            ...
            "total_contacts": len(self.config["email_settings"]["to_emails"]) + len(self.config["telegram_settings"]["chat_ids"])
        }
...
if __name__ == "__main__":
    ...
    # bot.set_email("your-email@gmail.com", username="your-email@gmail.com", password="your-app-password")
