...
class StreakTracker:
    def __init__(self):
        ...
        self.data_dir = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "data", "streaks"
        )
    ...
    def reset_streak(self, reason: str = "Manual reset") -> Dict:
        ...
        try:
            ...
            self.logger.log_recovery_action(
                "streak_reset",
                {"reason": reason, "previous_streak": data.get("current_streak", 0)},
            )
    ...
    def _get_streak_info(self, data: Dict) -> Dict:
        ...
        return {
            ...
            "days_to_next_milestone": (
                next_milestone - current_streak if next_milestone else None
            ),
        ...
        try:
            ...
            for milestone, message in sorted(self.milestones.items()):
                if milestone not in achieved_milestones and milestone > current_streak:
                    ...
                    if (
                        len([a for a in achievements if not a.get("unlocked", True)])
                        >= 3
                    ):
                        break
    ...
    def get_weekly_progress(self) -> Dict:
        ...
        try:
            ...
            for i in range(7):
                ...
                if last_clean:
                    last_clean_date = datetime.fromisoformat(last_clean).date()
                    if day <= last_clean_date and day >= last_clean_date - timedelta(
                        days=data.get("current_streak", 0) - 1
                    ):
                        is_clean = True
            ...
            return {
                ...
                "days": week_progress,
                "clean_days_this_week": sum(
                    1 for d in week_progress.values() if d["is_clean"]
                ),
            }
        ...
        try:
            ...
            if last_clean:
                ...
                while current_day <= today:
                    ...
                    if (
                        current_day <= last_clean_date
                        and current_day
                        >= last_clean_date - timedelta(days=streak_length - 1)
                    ):
            ...
            clean_percentage = (
                (clean_days_this_month / days_in_month) * 100
                if days_in_month > 0
                else 0
            )
            ...
            return {
                ...
                "longest_streak_this_month": min(
                    data.get("current_streak", 0), days_in_month
                ),
            }
