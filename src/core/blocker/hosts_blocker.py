    ...
    def block_domains(self, domains: List[str]) -> bool:
        ...
        try:
            ...
            if self._write_hosts_file(new_content):
                self.logger.log_blocking_action(
                    "hosts_block_domains", len(valid_domains), True
                )
                self.logger.info(
                    f"Successfully blocked {len(valid_domains)} domains in hosts file"
                )
                ...
                self.logger.log_blocking_action(
                    "hosts_block_domains", len(valid_domains), False
                )
                return False
    ...
    def remove_blocks(self) -> bool:
        ...
        try:
            ...
            if self._write_hosts_file(clean_content):
                ...
                self.logger.info(
                    "Successfully removed all blocking entries from hosts file"
                )
    ...
    def get_stats(self) -> dict:
        ...
        try:
            ...
            return {
                ...
                "hosts_file_size": (
                    os.path.getsize(self.hosts_path)
                    if os.path.exists(self.hosts_path)
                    else 0
                ),
