...
class DNSConfig:
    ...
    def get_network_interfaces(self) -> List[str]:
        ...
        try:
            ...
            if success and output.strip():
                ...
                self.logger.debug(
                    f"Found network interfaces via PowerShell: {interfaces}"
                )
                return interfaces
            else:
            ...
            if success:
                ...
                for line in lines:
                    ...
                    if (
                        ...
                        if interface_name and interface_name not in [
                ...
                self.logger.debug(
                    f"Found network interfaces via ipconfig: {interfaces}"
                )
                return interfaces
            else:
                ...
                success, output = self._run_command(
                    [
                        ...
                        (
                            ...
                            f"-AddressFamily IPv4 | Select-Object -ExpandProperty "
                            f"ServerAddresses"
                        ),
                    ]
                )
    ...
    def set_dns_servers(
        self, primary: str, secondary: str, interface: Optional[str] = None
    ) -> bool:
        """
        ...
        try:
            ...
            for iface in interfaces:
                ...
                if success:
                    success_count += 1
                    self.logger.info(
                        f"DNS set for interface {iface}: {primary}, {secondary}"
                    )
                else:
                    self.logger.warning(
                        f"Failed to set DNS for interface {iface}: {output}"
                    )
            ...
            if success_count > 0:
                ...
                self.logger.warning(
                    "DNS configuration failed - you may need to set DNS manually"
                )
                return False
    ...
    def test_dns_resolution(self, domain: str = "google.com") -> Dict[str, typing.Any]:
        """
        ...
        """
        try:
            success, output = self._run_command(["nslookup", domain])
            ...
            if success:
                ...
                return {"success": False, "domain": domain, "error": output}
