...
class Logger:
    def __init__(self, log_dir: Optional[str] = None):
        ...
        # Setup file paths
        self.log_file = os.path.join(self.log_dir, f"blocker_{datetime.now().strftime('%Y-%m')}.log")
        self.json_log_file = os.path.join(self.log_dir, f"blocker_{datetime.now().strftime('%Y-%m')}.json")
    ...
    def log(self, message: str, data: Optional[Dict[str, Any]] = None, level: str = "INFO"):
        """
    ...
    def log_blocking_action(self, action: str, domains_count: int = 0, success: bool = True):
    ...
    def log_recovery_action(self, action: str, details: Optional[Dict[str, Any]] = None):
    ...
    def get_daily_stats(self, date: Optional[datetime] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            stats = {
                ...
                "blocking_actions": len([log for log in daily_logs if "blocking" in log.get('message', '').lower()]),
                "recovery_actions": len([log for log in daily_logs if "recovery" in log.get('message', '').lower()]),
                "errors": len([log for log in daily_logs if log.get('level') == 'ERROR']),
